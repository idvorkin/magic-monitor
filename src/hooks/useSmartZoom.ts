        ...
        const detect = () => {
            ...
            if (video.currentTime !== lastVideoTimeRef.current) {
                ...
                if (result && result.landmarks && result.landmarks.length > 0) {
                    ...
                    const maxDim = Math.max(width, height * (video.videoWidth / video.videoHeight));
                    ...
                    currentZoomRef.current = currentZoomRef.current + (targetZoom - currentZoomRef.current) * smoothFactor;
                    currentPanRef.current.x = currentPanRef.current.x + (targetPanX - currentPanRef.current.x) * smoothFactor;
                    currentPanRef.current.y = currentPanRef.current.y + (targetPanY - currentPanRef.current.y) * smoothFactor;
                    ...
                    currentZoomRef.current = currentZoomRef.current + (1 - currentZoomRef.current) * (smoothFactor * 0.5);
                    currentPanRef.current.x = currentPanRef.current.x + (0 - currentPanRef.current.x) * (smoothFactor * 0.5);
                    currentPanRef.current.y = currentPanRef.current.y + (0 - currentPanRef.current.y) * (smoothFactor * 0.5);
